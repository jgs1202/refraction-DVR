...
//    field, center, radius, color, reflection, opacity, refraction, emissionColor
    float radius = WIDTH / 4, sCenter[] = {(float)WIDTH/2, (float)HEIGHT*2/3 - (float)WIDTH/20, radius + (float)WIDTH/10}, sColor[] = {1, 0.9, 0.9};
...
//            std::cout << lColor[(j + y * WIDTH + j * WIDTH * WIDTH)*3 + 2] << " ";
//        }
//    }
