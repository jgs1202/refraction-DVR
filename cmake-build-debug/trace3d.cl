...
__kernel void trace(__global float *grad, __global float *fColor, __global float *lColor,
 __global float *fOpacity, __global float *lIntensity,  __global float *pixel, int distance,
  int side, __global float *checker, int viewW, int viewH, __global float* rayPositions, __global float* rayAngles)
{
    int position = get_global_id(0);
    int x = getx(position, viewW), y = gety(position, viewW);
    unsigned long p;
    float color[] = {0., 0., 0.}, medium[] = {1.0, 1.0, 1.0}, angle[3], path[3], iniAngleX, lastAngleX;
    float newColor[3], newMedium[3], newIntensity, newGrad[3], square;   
    double opacity = 0, newOpacity;
    bool flagEnd = false;
    int state = 0, count = 0, ltf[3];
    float calcStep = 10;

    float xx = rayPositions[3 * position + 0];
    float yy = rayPositions[3 * position + 1];
    float z = rayPositions[3 * position + 2];

    angle[0] = rayAngles[3 * position + 0];
    angle[1] = rayAngles[3 * position + 1];
    angle[2] = rayAngles[3 * position + 2];
    ...
    float a = 0.001;
    checker[position] = color[0];//rayAngles[3 * position];//fColor[3 * ((int)(xx) + (int)(yy) * side + (int)(z) * side * side)];
