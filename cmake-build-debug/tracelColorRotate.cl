...
__kernel void trace(__global float *fColor, __global float *lColor, __global float *fOpacity,
 __global float *lIntensity,  __global float *pixel, int distance, int side, __global float *checker,
 int viewW, int viewH, __global float* rayPositions, __global float* rayAngles)
{
    int position = get_global_id(0);
    int x = getx(position, viewW), y = gety(position, viewW);
    float args[3], newColor[3], newMedium[3], newIntensity, newGrad[3], square;
